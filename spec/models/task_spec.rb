...
RSpec.describe 'タスクモデル機能', type: :model do
  ...
  describe '検索機能' do
    ...
    #     # expect(Task.title_search('task')).not_to include(second_task) #不要？
    #     # expect(Task.title_search('task').count).to eq 1 #不要？
    #   end
    # end

    # NG 文字列でない。レコードを探している。
    context 'scopeメソッドでステータス検索をした場合' do
      it "ステータスに完全一致するタスクが絞り込まれる" do
        expect(Task.status_sort('完了')).to include(task)
        expect(Task.status_sort('完了').count).to eq 1
        # expect(Task.status_sort('完了')).to include('完了: 3')
      end
    end
    ...
    #     # ここに内容を記載する
    #     expect(Task.search_sort('task1')).to include(task)
    #     expect(Task.status_sort('完了')).to include('完了')
    #   end
    # end
